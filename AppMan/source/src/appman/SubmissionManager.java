...
public class SubmissionManager implements SubmissionManagerRemote, Runnable
{
    ...
    private java.util.Vector tasksmanagerList;
    private int taskmanId = 0; // para gerar novos numeros de ids de TaskManagers
    private java.util.Vector graphs;
    ...
    public SubmissionManager(String id, String contact_address)
        {
            ...
            Debug.log(this+"\t[CONTACT]: "+contact_address);
            Debug.log("SubmissionManager created.");	
            static_die++;
    ...
    public float getTaskState(String taskId, String graphId)
        {
            return getGraph(graphId).getTask(taskId).getTaskState();
        }
    ...
    public void addGraph(Graph g)
        {	
            synchronized(graphs)
            {
            	if (graphs == null) Debug.log(this+"\tERRO  addGraph == graphs Null :-P");//PKVM
            	if (g == null) Debug.log(this+"\tERRO addGraph == g Null :-P");//PKVM
    ...
    private int computeSubmissonManagerTasksLoad()
    {
    	...
    	for(int i=0; i<tasksmanagerList.size(); i++)
    	{
    		...
    		num_tasks_running += tm.getTaskCount(Task.TASK_EXECUTING);
    	}
    ...
    private void executeGraph(String graphId)
        {		
            ...
            Debug.log(this+"\t****NUMERO DE TAREFAS NO SM:"+num_tasks_running+"\n");
            ...
            {            	
            	...
            	Debug.debug("SubmissionManager ["+this.getSubmissionManagerId()+"] executing graph [" + graphId + "] can manage "+ n +" more tasks", true);
            	...
            	else // sen�o adiciona todas
            	{
            		newreadytasks.addAll(allreadytasks);
            	}
            }
            else
            {
            	Debug.debug("SubmissionManager ["+this.getSubmissionManagerId()+"] executing graph [" + graphId + "] CANNOT MANAGE more tasks", true);
            }
            ...
            if(newreadytasks.size() > 0)
            {
                ...
                if(taskmanId < 1) // limita o numero de task managers      
                {
                    ...
                    taskmanId++; // novo id
                    tasksmanagerList.addElement(new TaskManager("taskman"+taskmanId));
                }
		 
                TaskManager taskman = (TaskManager)tasksmanagerList.lastElement();
            ...
            for(int i=0;i<alltasks.size();i++)
            {
                ...
                if(g.getTask(t.getTaskId()) == null)
                    if( ! isTaskFromHere(t.getTaskId())  ) // the task is from another submissiom manager
                    {
                        ...
                        try
                        {				
                            if(t.getTaskState() == Task.TASK_DEPENDENT)
                            {
                                t.setTaskState(Task.TASK_FOREIGN);
                            }
                            if(t.getTaskState() == Task.TASK_FOREIGN)
                            {
                                ...
                                for ( int needRetry = MAX_NUMBER_OF_RETRIES; needRetry>0; needRetry-- ) {
	                                try{
		                                if(appmanager.isTaskOutputsRemoteAvailable(t.getTaskId()))
		                                {
		                                    ...
		                                    t.setMySubmissionManagerRemoteContactAddress(gfsr);
		                                    t.setTaskState(Task.TASK_FOREIGN_FINAL);
		                                }
                                            //
	                                	...
	                                	Debug.log(this+"\tTrying isTaskOutputsRemoteAvailable - connect", e);
	                                }catch(java.net.SocketException e2) {
	                                	Debug.log(this+"\tTrying isTaskOutputsRemoteAvailable - socket", e2);
	                                }
                                    finally {
 ...
 */
    ...
    public synchronized String getSubmissionManagerId() //VDN 2005/01/13 sync
    	...
    	Debug.log(this+"\t[VINDN]submissionmanagerId: "+submissionmanagerId );
...
//	VDN:27/1/2006
	public long calculateDownloadTimeNow() throws RemoteException
	{
		  ...
		  for(int i=0; i<tasksmanagerList.size(); i++)
		  {
		  		plus += ((TaskManager)tasksmanagerList.elementAt(i)).getDownloadTimeOfTasks();
		  }

		  downloadTimeOfTasksManagers = plus;
 ...
 */
    public void setDieRemote() throws RemoteException
    	...
    	Debug.log(this+"\t[SM] SET ALL SM TO DIE: ");
            ...
            Debug.log(this+"\t[SM] Download Time: "+plus);
                ...
                if(graphs.size() > 0)
                {
                    ...
                    for(int i=0;i<graphs.size();i++)
                    {
                        ...
                        if(g.getStatePercentCompleted() < 1) // if the gragh is not complete executed
                        {
                            ...
                            {										
                                // executa o grafo
                            	Debug.log(this+"\t***[VINDN] executeGraph() GRAPHID: "+g.getGraphId());
