...
public class MyTask extends Task implements Serializable {
	private static final long serialVersionUID = -7613892980771100031L;
	...
	public void execute() throws Exception {
		...
		try {
			...
			setDownloadTimeOfFiles(finalTime - initialTime);
			Debug.log(this+"\t[VINDN] Download Time: " + (finalTime - initialTime));
			Debug.log(this+"\tMytask [" + this.getTaskId() + "] SET GridTask to RUN:" + gridtaskremote);
			// start remote execution
			gridtaskremote.setRun(true);

			//
			// XXX: trying to reduce remote pooling by using a blocking
			// primitive
			// with timeout
			//
			...
			while (true) {
				try {
					...
					Debug.debug("Failed to get remote task end status: " + e
							+ ". Will retry...");
				}
			}
	...
	private void transferOutputFiles() throws RemoteException {
		...
		if (this.getTaskType() == Task.TASK_TYPE_FINAL) {
			...
			Debug.debug("MyTask [" + this.getTaskId()
					+ "] Activator remote GridFileService: " + activator, true);

			h = AppManUtil.getExecutor().createObject(GridFileService.class,
					new Object[] {}, // a
					// seed
					// para o
					// servi�o
			...
			Debug
					.debug(
							...
							true);
			DataFile[] results_datafile = this.getOutputFiles();
	 ...
	 */
	private void downloadInputFiles() throws RemoteException, ConnectException,
			InterruptedException, MalformedURLException {

		Debug.debug("MyTask : starting method downloadInputFiles...", true);

		// TODO: neste metodo teria que verificar se esta em rede local, neste
		// caso
		// nao
		// precisaria transferir se estiver no NFS
		DataFile[] datafile = this.getInputFiles();
			...
			if (remote_task != null) {
				...
				while (true) {
					try {
						...
						if (remote_task.getTaskState() == Task.TASK_FOREIGN_FINAL) {
							...
							String contact_address_remote = remote_task
									.getMySubmissionManagerRemoteContactAddress();
							...
							Debug
									.debug(
											"MyTask ["
													...
													+ remote_task.getTaskId()
													+ "]: " + smr, true);
							buffer = smr.downloadFileFromGridTask(remote_task
									.getTaskId(), filepath);
						...
						Debug.debug("MyTask Task[" + this.getTaskId()
								...
								+ "] upload remote file to to GridTask.", true);
						break;
				...
				while (retry <= Task.MAX_RETRY_TIMES) {
					// if ((smr==null) || (
					// !((SubmissionManager)smr).ID.URLFileExists(filepath) ))
					// //VDN +
					// PKVM 2006/02/06
					if (!SubmissionManager.ID.URLFileExists(filepath))// VDN
					// //if(lastFilePath.compareTo(filepath) != 0)//VDN
					{
						Debug.debug(
								"MyTask - URLFileExists retornou false - retry="
										+ retry + " task=" + this.getName(),
								true);

						if ((filepath.indexOf("http") != -1)
								|| (filepath.indexOf("ftp") != -1)) {
							...
							Debug
									.debug(
											"MyTask - URLFileExists retornou true - retry="
													+ retry + " task="
													+ this.getName(), true);
						...
						// System.out.println("Download de Arquivo ja Feito,
						// copiando de
						// "+SubmissionManager.ID.getLocalPathFromURL(filepath));
						// Debug.debug("Download de Arquivo ja Feito, copiando
						// de
						// "+SubmissionManager.ID.getLocalPathFromURL(filepath),
						// true);
						if (copyFileFromDir(SubmissionManager.ID
								.getLocalPathFromURL(filepath)))
							break;
					}
	...
	/**
	 * PKVM 2006/02/06: method included to allow ImproveDownload to update its
	 * data structures
	 */
	private void storeCurrentURL(String filepath) {
		String localfile = filepath.substring(filepath.lastIndexOf("/") + 1);
	...
	private boolean downloadFileFromURL(String filepath)
		...
		try {
			...
			gridfileservice.installURLFile(filepath, localfile, false);
			// numDowloads++;
			// System.out.println("\t[VIND]NUMERO DE DOWNLOADS: "
			// + numDowloads);
			Debug.debug("Mytask [" + this.getTaskId()
					+ "] GridTask installURLFile Sucess OK: " + filepath
					+ " -> " + localfile, true);
			noError = true;
	...
	private boolean copyFileFromDir(String filepath)
	 ...
	 * @throws InterruptedException
	 */
	private void recoverConnectException(String filepath, MyTask remote_task,
			int retry, ConnectException e) throws ConnectException,
			InterruptedException {
		if (retry > 10) // tenta 10 vezes no maximo
		{
			/*
			 * Toler�ncia a Falhas Se a tarefa remota que possui o arquivo de
			 * depend�ncia falhar, ent�o aborta esta execu��o e seta a tarefa
			 * que falhou como DEPENDENT e seta o estado dos arquivos da tarefa
			 * que falhou como n�o existentes
			 */
			Debug.debug("Toler�ncia a Falhas - " + e, true);
			e.printStackTrace();
			remote_task.setAllOutputFileAsNotExist();
			remote_task.setTaskState(Task.TASK_DEPENDENT);
			// this.setTaskState(Task.TASK_DEPENDENT);
			Debug.debug("MyTask Task [" + remote_task.getTaskId()
					+ "] Error while transfering dependent files [" + filepath
	...
	private GeneralObjectActivator createGridTask(String where) throws RemoteException {
		...
		Debug.debug("MyTask [" + this.getTaskId() + "] create remote GridTask in Cell", true);

		GeneralObjectActivator activator = new GeneralObjectActivator(concreteTaskClassName,
					new Class[] { GridTaskRemote.class, GridFileServiceRemote.class }, 
					new String[] { "GridTaskRemote", "GridFileServiceRemote" }, 
					true);

		Debug.debug("MyTask [" + this.getTaskId() + "] Activator remote GridTask: " + activator, true);
	 ...
	 * @throws RemoteException
	 */
	private ObjectId createObjectId(String where,
			GeneralObjectActivator activator) throws RemoteException {
		ObjectId h = null;

		try {
			h = AppManUtil.getExecutor().createObject(
					Class.forName("appman." + concreteTaskClassName),
					new Object[] { (Task) this, this.getCommand_Line(), this.getName() }, 
					activator, 
					where);
		} catch (ClassNotFoundException cnfEx) {
			RemoteException rEx = new RemoteException(cnfEx.getMessage());
