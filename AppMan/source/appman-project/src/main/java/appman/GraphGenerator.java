...
public class GraphGenerator extends GraphLib
{
	...
	public static Vector<Object> getTaskList(Graph g)
	{
...
//		DAG_DSC dag = appdesc.getDAG();	//-->inserido por vindn
			   ...
			   for (TaskDescription t : appdesc.getListOfTasks()) {
				  ...
				  //           include an output datafile in the task "ti" ("ti" -> "t"), if the output datafile was not inserted yet					  
				   ...
				   if (t.getInputFiles() != null) {
					  for (String outfile : t.getInputFiles()) {

						 boolean found = false;
						 for (TaskDescription t_aux : appdesc.getListOfTasks()) {
							if (!(t_aux.equals(t)))
							{
						...
						// se o arquivo de entrada ? input da aplica??o, ou seja nao possui uma tarefa que gera o arquivo
						 if(!found) {
							String to_name = t.getTaskName();
							GraphNode to = (GraphNode)gnodes.elementAt(gnodes_names.indexOf(to_name));
							String filename = outfile;
	   ...
	   /////
	   for (TaskDescription t : (Vector<TaskDescription>) appdesc.getListOfTasks()) {
		   // outline of the algorithm:
		   //    for each task "t"
		   ...
		   //		   include an input datafile in the task "t" ("ti" -> "t")
		   //           include an output datafile in the task "ti" ("ti" -> "t"), if the output datafile was not inserted yet          
		   if (t.getOutputFiles() != null) {
			   for (String outfile : t.getOutputFiles()) {
				  boolean found = false;
				  for (TaskDescription t_aux : appdesc.getListOfTasks()) {
					 if (!t_aux.equals(t) && t_aux.hasInputFile(outfile)) {
						found = true;
						break;
					 }
				  }

				// se o arquivo de entrada é output da aplicação, ou seja nao possui uma tarefa que gera o arquivo
				  if(!found) {
				  	String filename = outfile;
					String to_name = t.getTaskName();
					GraphNode to = (GraphNode)gnodes.elementAt(gnodes_names.indexOf(to_name));
