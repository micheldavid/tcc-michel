 ...
 */
  ...
  String file;
    jj_consume_token(TRANSIENT);
       log.debug("[GRAND]\tTransient files:");
    label_8:
    while (true) {
      file = filename();
      log.debug(file);
  ...
  public SimpleParser(java.io.InputStream stream) {
    if (jj_initialized_once) {
    	log.debug("ERROR: Second call to constructor of static parser.  You must");
    	log.debug("       either use ReInit() or set the JavaCC option STATIC to false");
    	log.debug("       during parser generation.");
      throw new Error();
    }
